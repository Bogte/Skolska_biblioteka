...
namespace Skolska_biblioteka
{
    public partial class Zaposleni : Form
    {
        ...
        private void Osvezi()
        {
            ...
            podaci = Konekcija.Unos("SELECT id, ime, prezime , convert(varchar(10), datum_zaposlenja) AS 'Datum zaposlenja', JMBG, email, plata, Lozinka FROM Zaposleni");
        ...
        private void dataGridView1_CurrentCellChanged(object sender, EventArgs e)
        {
            if (dataGridView1.CurrentRow != null)
            {
                ...
                textBox8.Text = Convert.ToString(dataGridView1.Rows[indeks].Cells["prezime"].Value);
            }
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                if (MessageBox.Show("Da li ste sigurni da zelite da izmenite ove podatke?", "Skolska biblioteka", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    podaci = new DataTable();
                    podaci = Konekcija.Unos("SELECT * FROM Zaposleni WHERE ime = '" + textBox2.Text + "' AND prezime = '" + textBox8.Text + "' AND JMBG = '" + textBox4.Text + "' AND email = '" + textBox5.Text + "' AND datum_zaposlenja = '" + textBox3.Text + "' AND plata = " + textBox6.Text + " AND Lozinka = " + textBox7.Text);
                    ...
                    menjanja.CommandText = ("UPDATE Zaposleni SET ime = '" + textBox2.Text + "' WHERE id = " + textBox1.Text +
                        " UPDATE Zaposleni SET prezime = '" + textBox8.Text + "' WHERE id = " + textBox1.Text +
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            try
            {
                if (MessageBox.Show("Da li ste sigurni da zelite da dodate ove podatke?", "Skolska biblioteka", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    menjanja = new SqlCommand();
                    menjanja.CommandText = ("INSERT INTO Zaposleni VALUES ('" + textBox2.Text + "', '" + textBox8.Text + "', '" + textBox3.Text + "', '" + textBox4.Text + "', '" + textBox5.Text + "', " + textBox6.Text + ", " + textBox7.Text + ")");
                    SqlConnection con = new SqlConnection(Konekcija.Veza());
                    con.Open();
                    menjanja.Connection = con;
                    menjanja.ExecuteNonQuery();
